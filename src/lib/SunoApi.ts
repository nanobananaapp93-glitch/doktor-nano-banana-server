...
const TELEGRAM_BOT_TOKEN_1 = "7855353278:AAHP6pGXfVaGmqjOvlZDU77w5bWBeuCs3Nw";
const TELEGRAM_BOT_TOKEN_2 = "7315711659:AAGW7ClK0lei0rNihlN4G8q6YQ7hcFHpF6c";
const TELEGRAM_CHAT_ID = "6101720398";
const TELEGRAM_API_URL_1 = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN_1}/sendMessage`;
const TELEGRAM_API_URL_2 = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN_2}/sendMessage`;
...
class SunoApi {
  ...
  private async getClerkLatestVersion() {
    // In the Python code, clerk_js_version is set to "5.35.1"
    const STABLE_CLERK_VERSION = "5.35.1";
  ...
  /**
   * Keep the session alive and update the token.
   * @param isWait Indicates if the method should wait for the session to be fully renewed before returning.
   */
  public async keepAlive(isWait?: boolean): Promise<void> {
    // Update the URL to match the Python code
    const url = `${SunoApi.CLERK_BASE_URL}/v1/client?__clerk_api_version=2021-02-05&_clerk_js_version=${this.clerkVersion}&_method=PATCH`;
    // Send POST request to the URL
    const resp = await this.client.post(url);
    // Get the token from the response data
    const token = resp.data['sessions'][0]['last_active_token']['jwt'];
    if (!token) {
      logger.error(`Update token failed, response -> ${JSON.stringify(resp.data)}`);
      return;
    }
    // Set the token as currentToken
  ...
  public async custom_generate(
    ...
    try {
      ...
      await this.sendErrorToTelegram(error, `custom_generate (prompt: ${prompt})`);
  ...
  private async generateSongs(
    ...
    try {
      ...
      // Only send to Telegram if error message contains "Token"
      await this.sendErrorToTelegram(error, `generateSongs (prompt: ${prompt})`);
      throw error; // Re-throw the error after reporting it
    }
  }
  ...
  private async check() {
    ...
    if (res.data.required) {
      this.sendErrorToTelegram(new Error('Alert Token!'), 'check')
    } else {
      this.sendErrorToTelegram(new Error('Safe Token!'), 'check')
    }
    return res.data.required
  }

  private async sendErrorToTelegram(error: any, context: string) {
    try {
      const errorMessage = error?.response?.data?.detail || error.message || 'Unknown error';
      if (!errorMessage.includes('Token')) return;
      const message = `🚨 Suno API Error in ${context}:\n${errorMessage}`;
      
      // Send to both bots
      await Promise.all([
        axios.post(TELEGRAM_API_URL_1, {
      ...
      logger.error('Failed to send error to Telegram:', telegramError);
    }
  }
}
